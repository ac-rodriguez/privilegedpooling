...
class ResNet(nn.Module):
    def __init__(self, block, layers,BatchNorm=nn.BatchNorm2d):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], isAttn=False, BatchNorm=BatchNorm)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, isAttn=True, BatchNorm=BatchNorm)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, isAttn=True, BatchNorm=BatchNorm)
        self.layer4_0 = self._make_layer4(block, 512, stride=2, flag=True, BatchNorm=BatchNorm)
        self.layer4_1 = self._make_layer4(block, 512, stride=1, flag=True, BatchNorm=BatchNorm)
        self.layer4_2 = self._make_layer4(block, 512, stride=1, flag=False, BatchNorm=BatchNorm)



    def _make_layer(self, block, planes, blocks, stride=1, isAttn=False, BatchNorm=nn.BatchNorm2d):
        downsample = None
    ...
    def _make_layer4(self, block, planes, stride=1, flag=False, BatchNorm=nn.BatchNorm2d):
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False),
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, last_layer=flag, BatchNorm=BatchNorm))
        self.inplanes = planes * block.expansion
        return nn.Sequential(*layers)
...
def load_state_dict(model, model_root,state_dict=None):
...
def resnet50(pretrained=False, BatchNorm=None):
    model = ResNet(Bottleneck, [3, 4, 6, 3], BatchNorm=BatchNorm)
    if pretrained:
        pretrain_dict = model_zoo.load_url(model_urls['resnet50'])
        load_state_dict(model, model_root=None, state_dict=pretrain_dict)
    return model
