...
class MPNCOV(nn.Module):
     ...
     def _cov_pool_atn(self, x):
         return CovpoolAttn.apply(x)
     def _cov_pool_atn_weighted(self, x, attention):
         return CovpoolAttn_weighted.apply(x, attention)
     def _sqrtm(self, x):
...
class CovpoolAttn(Function):
    @staticmethod
    def forward(ctx, input):
        x = input
        B, dim, a, h, w = x.data.shape
        M = h * w * a
        x = x.reshape(B, dim, M)
        x_c = x - x.mean(dim=-1, keepdim=True)
        y = x_c.bmm(x_c.transpose(1, 2)) * (1 / M)
        ctx.save_for_backward(x_c, input)
        return y

    @staticmethod
    def backward(ctx, grad_output):
        x_c, input = ctx.saved_tensors
        B, dim, a, h, w = input.data.shape
        M = h * w * a
        grad_input = grad_output + grad_output.transpose(1, 2)
        grad_input = grad_input.bmm(x_c) / M
        grad_input = grad_input.reshape(B, dim, a, h, w)
        return grad_input


class CovpoolAttn_weighted(Function):
    @staticmethod
    def forward(ctx, input, atn_map):
        x = input
        B, dim, a, h, w = x.data.shape
        M = h * w * a
        x = x.reshape(B, dim, M)
        atn_map = atn_map.reshape(B,1, M)
        with torch.no_grad():
            N = atn_map.sum(dim=-1, keepdim=True)
        x_mean = x.sum(dim=-1, keepdim=True) / N
        x_c = x - x_mean*atn_map

        y = x_c.bmm(x_c.transpose(1, 2)) * (1 / N)
        ctx.save_for_backward(x_c, input, N)
        return y

    @staticmethod
    def backward(ctx, grad_output):
        x_c, input, N = ctx.saved_tensors
        B, dim, a, h, w = input.data.shape
        grad_input = grad_output + grad_output.transpose(1, 2)
        grad_input = grad_input.bmm(x_c) / N
        grad_input = grad_input.reshape(B, dim, a, h, w)
