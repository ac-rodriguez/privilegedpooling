...
class Basemodel(nn.Module):
    ...
    def get_lr_params(self, keyword='backbone', inclusive=True):
        ...
        if keyword=='backbone' and inclusive:
            params_list = [self.features.parameters()]
            params_list.append(self.representation.parameters())
        elif keyword=='backbone' and not inclusive:
            params_list = [self.classifier.parameters()]
        else:
            raise NotImplementedError

        for params_gen in params_list:
            for el in params_gen:
                if el.requires_grad:
                    yield el