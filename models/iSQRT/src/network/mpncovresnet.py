...
import torch
from models.iSQRT.src.network.resnet import Bottleneck
...
def mpncovresnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        if 'pf-pc' in socket.gethostname():
            model_name = "/scratch/iSQRT_pretrained/mpncovresnet50-15991845.pth"
        else:
            model_name = "/cluster/work/igp_psr/sdaronco/tmp/mpncovresnet50-15991845.pth"
        model.load_state_dict(torch.load(model_name)) #  model_zoo.load_url(model_urls['mpncovresnet50']))
...
def mpncovresnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        if 'pf-pc' in socket.gethostname():
            model_name = "/scratch/iSQRT_pretrained/mpncovresnet101-ade9737a.pth"
        else:
            model_name = "/cluster/work/igp_psr/sdaronco/tmp/mpncovresnet101-ade9737a.pth"
        model.load_state_dict(torch.load(model_name)) #  model_zoo.load_url(model_urls['mpncovresnet50']))
